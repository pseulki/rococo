...
def evaluation(model, data_loader, device, config):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_embeds.append(text_embed)    # 0221 .cpu()
    ...
    score_matrix_i2t = torch.full((len(data_loader.dataset.image),len(texts)),-100.0).to(device) #len image x text (e.g., 5000 x 50010)
    ...
    for i,sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)): #0221 todevice
...
def main(args, config):
    ...
    score_test_i2t = evaluation(model_without_ddp, test_loader, device, config)

    if utils.is_main_process():  
        test_result = itm_eval(score_test_i2t, test_loader.dataset.img2txt, test_loader.dataset.wrongtext)
        print(test_result)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--testfilename', default='rand_voca.json', type=str)
